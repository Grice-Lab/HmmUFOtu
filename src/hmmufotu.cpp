...
int main(int argc, char* argv[]) {
	...
	if(chiOut.is_complete())
		chiOut << "id\tdescription\t" << HmmAlignment::TSV_HEADER << "\treplication_rate\t" + PTPlacement::TSV_HEADER << endl;
...
#pragma omp critical(writeAln)
						...
						if(!alignOnly) {
							...
							/* get potential locs */
							vector<PTLoc> locs = getSeed(ptu, seq, aln.csStart - 1, aln.csEnd - 1, maxDiff);
							...
							if(checkChimera) {
								...
								cerr << "bestPlace.loglik: " << bestPlace.loglik << endl;
								cerr << "bestPlace.taxon: " << bestPlace.getTaxonName() << endl;
								double loglik = 0;
...
//									cerr << seq.getName() << " " << fwdRead.getDesc() << " seg " << i << " pDist: " << segPDist << endl;
									...
									loglik += segPlaces[0].loglik;
									cerr << "seg " << i << ": " << segPlaces[0].getTaxonName() << endl;
								}
								cerr << seq.getName() << " " << fwdRead.getDesc() << " delta.loglik: " << loglik - bestPlace.loglik << endl;
							}
						} /* end if alignOnly */
						/* write main output */
