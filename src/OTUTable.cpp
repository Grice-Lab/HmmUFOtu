...
istream& OTUTable::loadTable(istream& in, const string& progName, const VersionSequence& progVer) {
	string line;
	while(in.peek() == '#') { /* comment header line */
		std::getline(in, line);

		char pname[MAX_NAME_LENGTH];
		char ver[MAX_NAME_LENGTH];
		int scanned = sscanf(line.c_str(), "# OTU table generated by %s %s", pname, ver);
		if(scanned != 2)
			continue; /* other comment line */

		if(progName != pname) { /* a header line */
			errorLog << "Not an OTU Table file" << endl;
...
ostream& OTUTable::saveTable(ostream& out, const string& progName, const VersionSequence& progVer) const {
	/* output header */
	out << "# OTU table generated by " << progName << " " << progVer << endl;
