...
void printUsage(const string& progName) {
	...
	cerr << "Usage:    " << progName << "  <HmmUFOtu-DB> [INFILE [INFILE2 ...]] <-o OTU-OUT> [options]" << endl
		 ...
		 << "            --hmm-iden  DBL    : minimum profile-HMM identity (proportion of non-gapped bases of profile) required for assignment result [" << DEFAULT_MIN_HMM_IDENTITY << "]" << endl
...
int main(int argc, char* argv[]) {
	...
	writeProgInfo(otuOut, string(" OTU table generated by ") + argv[0]);
	otuTable.save(otuOut, TABLE_FORMAT);

	/* write read list */
	if(readOut.is_open()) {
		infoLog <<"Wring read ID lists" << endl;
		writeProgInfo(readOut, string("OTU read info generated by ") + argv[0]);
